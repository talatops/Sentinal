...
class TriggerTrivyScan(Resource):
    ...
    def post(self):
        ...
        # Create or update run
        run = CICDRun.query.filter_by(commit_hash=data.get('commit_hash', 'manual')).order_by(CICDRun.created_at.desc()).first()
        if not run:
            run = CICDRun(
                commit_hash=data.get('commit_hash', 'manual'),
                branch=data.get('branch', 'main'),
...
class ScanStatus(Resource):
    ...
    def get(self, scan_type, scan_id):
        ...
        if scan_type == 'zap':
            ...
            return {'status': results.get('status'), 'progress': results.get('active_scan_progress', 0), 'results': results}, 200
        else:
