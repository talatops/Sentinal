    ...
    def get(self):
        ...
        try:
            ...
            curl_cmd = [
                ...
                '-d', f'client_id={github_client_id}',
                '-d', f'client_secret={github_client_secret}',
                '-d', f'code={code}',
                '--connect-timeout', '30',
                '--max-time', '30'
            ]
            ...
            if result.returncode != 0:
                ...
                return {'error': 'Failed to connect to GitHub. Please check your internet connection and DNS settings.'}, 503
        ...
        if 'error' in token_data:
            current_app.logger.error(f'GitHub OAuth error: {token_data.get("error_description", token_data.get("error"))}')
            return {'error': f'GitHub OAuth failed: {token_data.get("error_description", token_data.get("error"))}'}, 400
        ...
        try:
            ...
            if result.returncode != 0:
                current_app.logger.error(f'Curl failed: {result.stderr}')
                return {'error': 'Failed to connect to GitHub API. Please check your internet connection and DNS settings.'}, 503
            ...
            return {'error': f'An error occurred while fetching user info from GitHub: {str(e)}'}, 500
