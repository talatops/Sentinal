...
class ThreatDetector:
    ...
    def detect_threats_from_scan(self, scan_run_id: int) -> List[Dict[str, Any]]:
        ...
        if scan_run.sast_results:
            threats_created.extend(
                self._process_sonarqube_results(scan_run.sast_results, scan_run_id)
            )

        # Process OWASP ZAP (DAST) results
        if scan_run.dast_results:
            threats_created.extend(
                self._process_zap_results(scan_run.dast_results, scan_run_id)
            )

        # Process Trivy results
        if scan_run.trivy_results:
            threats_created.extend(
                self._process_trivy_results(scan_run.trivy_results, scan_run_id)
            )
    ...
    def _create_or_link_threat(
        ...
        try:
            ...
            if existing_link:
                ...
                return {
                    'action': 'linked',
                    'threat_id': existing_link.threat_id,
                    'vulnerability_id': existing_link.id
                }
            ...
            return {
                'action': 'created',
                'threat_id': threat.id,
                'vulnerability_id': threat_vuln.id
            }
