...
const ThreatDiagram = ({ threat, onNodeClick }) => {
  const { nodes, edges } = useMemo(() => {
    ...
    if (
      flow.includes("database") ||
      flow.includes("db") ||
      flow.includes("postgres") ||
      flow.includes("mysql")
    ) {
    ...
    if (
      flow.includes("auth") ||
      flow.includes("login") ||
      flow.includes("authentication")
    ) {
      components.push({ type: "auth", name: "Auth Service", icon: "🔐" });
    }
    if (
      flow.includes("frontend") ||
      flow.includes("ui") ||
      flow.includes("web")
    ) {
    ...
    components.forEach((comp, index) => {
      const position = nodePositions[comp.type] || {
        x: 300 + index * 200,
        y: 200,
      };
    ...
    return (
      <div className="flex items-center justify-center h-96 bg-cyber-dark rounded-lg border border-gray-700">
        <p className="text-gray-400">
          Select a threat to view its data flow diagram
        </p>
      </div>
    );
  }
